...
with topic.get_producer(delivery_reports=True) as producer2:
    count = 0
    while True:
        count += 1
        producer.produce(datetime.now().strftime("%Y%m%d %H:%M:%S")+' p2 test message '+str(count), 
        	partition_key='{}'.format(count))
        if count % 10**5 == 0:  # adjust this or bring lots of RAM ;)
            while True:
                try:
...
with topic.get_producer(delivery_reports=True) as producer3:
    count = 0
    while True:
        count += 1
        producer.produce(datetime.now().strftime("%Y%m%d %H:%M:%S")+' p3 test message '+str(count), 
        	partition_key='{}'.format(count))
        if count % 10**5 == 0:  # adjust this or bring lots of RAM ;)
            while True:
                try:
